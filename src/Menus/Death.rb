module Menus
  class Death < AdventureRL::Menu
    ...
    private
      ...
      def get_buttons
        return {
          retry:   Buttons::DeathRetry.new(@buttons_settings.merge(@settings.get(:retry))),
          to_main: Buttons::DeathToMain.new(@buttons_settings.merge(@settings.get(:to_main)))
        }
      end
